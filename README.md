...
输入主题或文档，AI 自动生成可编辑的 PPTX；或输入描述 + 选风格，AI 绘制插画。前后端分离、异步任务编排的全栈项目。
...
完整流程：**输入中文描述 → LLM 优化为英文生图提示词 → 火山方舟 Seedream 生图 → 下载校验 → 持久化存储**
...
- 多种输入：主题 / 长文本 / PDF / Word / Markdown / TXT
- 大纲先行：先生成结构化大纲，标题、要点、顺序均可调整
- 并发生成：每页拆成独立 ARQ 任务，SSE 实时推送进度，单页可重试
- 在线编辑：缩略图 + 16:9 画布 + AI 修改 / 主题 / 版式面板
- 原生导出：python-pptx 构造可编辑对象，导出前质量门禁（error/warning 分级）
